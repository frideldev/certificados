...
function init(){
    ...
    $("#usuario_file_form").on("submit",function(e){
        guardaryeditarimg(e);
    });
}
            ...
            workbook.SheetNames.forEach(function(sheetName) {
                ...
                for (i = 0; i < UserList.length; i++) {
                    ...
                    $.post("../../controller/usuario.php?op=guardar_desde_excel",{
                        usu_nomapm : columns[0],
                        usu_correo : columns[1],
                        usu_pass : columns[2],
                        usu_grado : columns[3],
                        usu_ciudad : columns[4],
                        usu_telf :columns[5],
                        usu_img :columns[6],
                        rol_id : columns[7],
                        usu_dni :columns[8]
...
function guardaryeditarimg(e){
    e.preventDefault();
    var formData = new FormData($("#usuario_file_form")[0]);
    $.ajax({
        url: "../../controller/usuario.php?op=update_imagen_usuario",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
