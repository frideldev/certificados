...
function init(){
    ...
    $("#detalle_form_firmado").on("submit",function(e){
        guardaryeditarimgfirmado(e);
    });
}
...
function editar(cur_id){
    $.post("../../controller/curso.php?op=mostrar",{cur_id : cur_id}, function (data) {
        ...
        $('#vigencia').val(data.vigencia).trigger('change');
        $('#cur_pago').val(data.cur_pago).trigger('change');
...
function imagenfirmado(cur_id){
    $('#curx_idx2').val(cur_id);
    console.log("hola");
    $('#modalfilefirmado').modal('show');
}
function nuevo(){
...
function guardaryeditarimgfirmado(e){
    e.preventDefault();
    var formData = new FormData($("#detalle_form_firmado")[0]);
    $.ajax({
        url: "../../controller/curso.php?op=update_imagen_curso_firmado",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
